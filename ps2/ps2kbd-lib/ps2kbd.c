...
static uint kdb_pio_irq; 
static uint kbd_sm;         // pio state machine index
static uint kbd_connected;
...
void kbd_init(uint pio, uint gpio) {
    ...
    kdb_pio_irq = pio ? PIO1_IRQ_0 : PIO0_IRQ_0;
...
//configure out section PS2
    ...
    pio_sm_set_consecutive_pindirs(kbd_pio, kbd_sm, gpio, 2, false);
    // Set the out pins
...
//start the pins as input at start
    ...
    pio_gpio_init(kbd_pio, dataPin);
    pio_gpio_init(kbd_pio, clkPin);
    ...
    irq_set_exclusive_handler(kdb_pio_irq, kbd_keyboard_event);
    irq_set_enabled(kdb_pio_irq, true);
